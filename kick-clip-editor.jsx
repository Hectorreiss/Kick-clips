...
export default function KickClipEditor() {
  ...
  const handleLoadClip = () => {
    ...
    setImportError("");
    setResolving(true);
    // Tenta carregar o link colado diretamente como vídeo, no navegador.
    // Só funciona se o link apontar para o arquivo de vídeo em si (ex.
    // termina em .mp4/.m3u8) e o servidor permitir acesso entre origens.
    // Para a página comum de um clipe (kick.com/streamer?clip=...), isso
    // normalmente não é possível sem um backend por trás para resolver o
    // link — nesse caso mostramos o motivo em vez de travar carregando.
    clearTimeout(resolveTimeoutRef.current);
    resolveTimeoutRef.current = setTimeout(() => {
      setResolving(false);
      setImportError(
        "Não foi possível carregar esse link diretamente nesta prévia (sem servidor por trás para resolvê-lo). Em produção, o backend cuida disso automaticamente."
      );
    }, 7000);
  };
  ...
  const onProbeError = () => {
    ...
    setResolving(false);
    setImportError(
      "Não conseguimos carregar esse link diretamente nesta prévia (sem servidor por trás para resolvê-lo). Em produção, o backend busca o clipe automaticamente pela API da Kick."
    );
  };
      ...
      <div className="min-h-screen w-full bg-neutral-950 text-neutral-200 flex flex-col">
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-6 py-16">
          ...
          <div className="max-w-xl w-full bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
            ...
            {resolving && (
              <video
                ref={probeRef} src={clipUrl} crossOrigin="anonymous" muted className="hidden"
                onLoadedMetadata={onProbeReady} onError={onProbeError}
              />
            )}
            ...
            <p className="text-[11px] text-neutral-600 mt-4 leading-relaxed flex gap-1.5">
              ...
              Em produção, o Kick Clip Editor busca o vídeo automaticamente pelo link, pela API pública da Kick, sem exigir login. Esta prévia roda sem servidor: ela tenta carregar o link direto no navegador, e se não conseguir, avisa o motivo — nunca abre o seletor de arquivos do computador sozinha.
            </p>
            <button
              onClick={() => importFileRef.current?.click()}
              className="text-[11px] text-neutral-600 hover:text-violet-400 underline underline-offset-2 mt-2"
            >
              Prefere testar com um arquivo de vídeo seu? Selecione manualmente.
            </button>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="min-h-screen w-full bg-neutral-950 text-neutral-200 flex flex-col select-none">
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@500;600;700&display=swap');`}</style>
      <video ref={videoRef} src={videoSrc} className="hidden" onLoadedMetadata={onEditorMeta} playsInline />
            ...
            {mode !== "original" && (
              <div className="absolute inset-0">
                {layerOrder.map((id) => {
                  ...
                  if (!l) return null;
                  return (
                    <LayerBox key={id} id={id} box={l} color={l.type === "text" ? "#fbbf24" : "#34d399"}
                      label={l.type === "text" ? "Texto" : "Overlay"}
            ...
            {mode === "916" && selectedBox && (
              <div>
                <div className="text-[11px] uppercase tracking-wide text-neutral-500 mb-2">
                  {selected === "camera" ? "Câmera" : selected === "gameplay" ? "Gameplay" : selectedBox.type === "text" ? "Texto" : "Overlay"}
                </div>
          ...
          <div className="p-4 flex-1">
            ...
            <div className="space-y-1.5">
              {[...layerOrder].reverse().map((id) => {
                ...
                const label = isCam ? "Câmera" : isGame ? "Gameplay" : l?.type === "text" ? (l.text || "Texto") : "Overlay";
                const Icon = isCam ? CameraIcon : isGame ? Video : l?.type === "text" ? Type : ImageIcon;
